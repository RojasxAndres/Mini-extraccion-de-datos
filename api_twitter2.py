...
if __name__=='__main__':
    encoding='UTF-8'
    archivo_datos=io.open("JorgeIvanOspina_urls.txt","w",encoding=encoding)

    consumer_key = "nURCWrpkAOBvxsxCtqisrw7TI"
    consumer_secret = "48znd0KtCGWtH9wheKXtSHuF8ngKFdQ0q9ETPPdk2cDyT8lgZR"
    access_key = "1153699662894944256-RJaXen4wGbWhjYd2RX1rWgEeJLTVpi"
    access_secret ="TqD8OY1mb8JDBOhd7yGsM9NTpyO21VEntDoN96c1NAYhg"
    ...
    for status in tweepy.Cursor(api.user_timeline,id="JorgeIvanOspina",tweet_mode='extended_entities').items():
        #pprint(status._json)
    ...
    print(len(lista_url))
    indice=0
    for status in tweepy.Cursor(api.user_timeline,id="JorgeIvanOspina",tweet_mode='extended').items(len(lista_url)):
        #pprint(status._json)
        hashtags=""
        mentions=""
